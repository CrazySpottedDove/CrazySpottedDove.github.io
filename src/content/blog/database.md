...
我们用各种范式来衡量一个数据库系统设计的优劣。

### 第一范式 First Normal Form

首先，我们这样定义值的原子性：

> 如果一个域的元素都是不可分的，那么我们说，这个域是原子的。

举例说，复合属性(Composite attributes)，多值属性(Multi-value attribute)都不是原子的。

在原子性的基础上，第一范式(1NF)这样定义：

> 如果一个关系模式 R 的所有域都是原子的，那么我们说，R 是符合第一范式的。
>
> 也就是说，表里面每一个属性的值都是不可分的。
...
>
> 而且，如果这么做，我们就要在应用程序中写额外的逻辑来提取一个值中额外的信息。
>
> 一个例子就是火灾浙大中为了提取课程的学期和学年，不得不把雪灾浙大 api 中类似 `2023-2024春夏` 的值手动拆开，这种感觉非常不好。

### 关系型数据库设计中的陷阱

如果不好好设计关系型数据库，就很可能引入数据冗余。同时，还可能使得数据更新变得复杂。
...
Main memory 拥有较快的访问速度，但对于整个数据库来说往往容量不够。它也具有易失性。

Flash memory/Electrically Erasable Programmable Read-Only memory 的访问速度和主存相当，但是写和擦除数据的速度较慢。数据只可以在某个位置写一次，不过这个位置的数据可以被擦除。闪存的数据具有非易失性。闪存的单位存储开销与主存相当，并在嵌入式设备中广泛使用（数码相机，手机，USB）。

Magnetic disk 是长期存储的数据的主要存储介质，其中任何需要访问的数据必须先被移动到主存。它的数据具有非易失性，且物理错误也很少见。

Optical storage 多为一次写入，之后多次读取(WORM)的存储。也有一些允许多次写入的光盘，如 DVD-RAM。它的读写速度比磁盘更慢。

Tage sorage 主要用于备份，具有非易失性，容量非常大，读写速度则远低于磁盘。
...
磁盘控制器是计算机系统与磁盘驱动硬件之间的接口，负责管理磁盘的读写操作。其功能如下：

1. **接受高层命令**：
   - 接收计算机系统发出的高层命令（如读取或写入某个扇区）。
...
3. **校验数据完整性**：
   - 计算并附加校验和（Checksum）到每个扇区。
   - 读取数据时重新计算校验和，验证数据是否正确。
   - 如果数据损坏，校验和将不匹配，从而检测到错误。
...
磁盘控制器通过这些功能确保磁盘操作的可靠性和数据完整性，同时屏蔽底层硬件的复杂性，为计算机系统提供统一的接口。
...
MTTF 是指磁盘预计能够持续工作，不发生错误的平均时间。一般来说，在 3-5 年，且随着磁盘的老化而减小。

#### Optimization of Disk-Block Access

Block 是磁道中的一个连续的扇区序列，它是磁盘与主存数据传输的最小单元。

为了减少磁盘访问的寻道时间，我们引入磁盘臂调度算法（Disk-arm-scheduling algorithms）。

电梯算法就是一种磁盘臂调度算法。它让磁盘臂向某个方向移动，并在移动过程中处理这个方向上的所有请求。如果没有请求了，则反转方向，继续处理另一方向上的所有请求。这个过程和电梯的运转方式很像，因此得名“电梯算法”。

![alt text](mdPaste/database/image-27.png)

我们也可以在文件的组织方式上做优化。我们把相关的信息放在同一个或相邻的柱面上，以减少寻道时间和旋转延迟。

随着文件中不断插入或删除数据，文件块可能变得分散。这导致磁盘上的空闲块分布不连续，新创建的文件也就可能会被分散存储。这就是文件碎片化（File Fragmentation）的现象。

对碎片化文件的顺序访问会导致磁盘臂频繁移动，从而增长访问时间。一些系统提供了碎片处理工具，用于重新组织文件块。不过，大多数系统在进行碎片处理时是不可用的。

非易失性写缓冲区也可以加快磁盘写入操作。当发生数据写入时，优先写入到非易失性的 RAM 中。磁盘控制器在磁盘没有别的请求时执行这些写入。

日志磁盘（Log Disk）是专门用于记录块更新操作的顺序日志的磁盘。日志磁盘的写入较快，因为它是顺序写入，不需要磁盘寻道。

为了提高性能，文件系统通常会对写入操作进行重排序。如果没有日志记录，系统崩溃时，写入重排序可能导致文件系统数据损坏。这正是日志磁盘的作用。

### RAID

RAID 是独立磁盘冗余阵列(Redundant Arryas of Independent Disks) 的缩写。它是一种磁盘组织技术，通过管理大量磁盘，提供以下特性：

1. **高容量和高速度**：
   - 通过多个磁盘并行工作，提升存储容量和数据传输速度。
2. **高可靠性**：
   - 通过冗余存储数据，即使某个磁盘发生故障，也能恢复数据，避免数据丢失。

#### redundancy

冗余旨在通过存储额外的信息来保证出现磁盘故障时可以恢复原有数据。
...
level 0 使用块级拆分，并且没有数据冗余。它常用于一些强调高性能，且数据丢失并不重要的应用中。

##### Level 1

level 1 使用块级拆分，同时使用了镜像存储。

##### Level 2

level 2 使用比特级拆分，并使用额外的错误校验码(Error-Correcting-Codes)。

##### Level 3

level 3 也使用比特级拆分，并使用单一的奇偶校验位(Parity Bit) 来进行错误检测和纠正。

在 level 3 中，数据按位分布到多个磁盘上，并在每次写入时计算出它的奇偶校验位，放在一个专门的 parity bit disk。如果某个磁盘损坏了，就通过其他磁盘的数据和奇偶校验位计算恢复丢失的数据。

level 3 的数据传输较快，但是由于每次 IO 都需要所有磁盘的参与，所以 IO 并发能力较低。level 3 是 level 2 的优化版本。

##### Level 4

level 4 使用块级拆分，并使用块级的校验位。level 4 用一个单独的 parity disk 来存储校验位，且将校验位用块来组织，形成校验块。逻辑上，每当写入一个 data block 时，其它对应的数据块和它取异或的结果被写入校验块。

而实际操作上，在写入的数据块较少时，可以使用旧的 parity block，旧的 data block 和新的 data block 取异或。这样做总共有两次块读取（读老的）和两次块写入（读新的）。
...
而当需要写入大量连续数据时，可以直接用所有新数据块重新计算奇偶校验块。这样做可以避免逐块读取旧的奇偶校验块和数据块。

相较于 level 3，level 4 的 IO 性能更高，这是因为读的操作允许以块来进行，多个磁盘中的块的读取就可以并行操作。

每次写入单个数据块时，都需要对奇偶校验块进行更新，这便是 level4 中独立块写入操作的性能瓶颈。

##### Level 5

level 5 使用块级拆分，但把校验块分散在 $N+1$ 个磁盘中。

![alt text](mdPaste/database/image-28.png)

这样的设计允许 level 5 比 level 4 拥有更高的 IO 性能。因为校验块分布在多个磁盘上，涉及不同磁盘的写入可以并行进行，而不需要集中访问单一的校验盘。

##### Level 6

level 6 使用块级拆分，并在 level 5 的基础上添加了额外的冗余信息，能够同时应对多块磁盘故障。它的读取性能与 level 5 相当，写入性能则由于需要计算和写入额外的冗余信息而有所下降。
